...
app.get('/api/admin/trip/:tripId/bookings', (req, res) => {
...
app.post('/api/admin/confirm-payment', requireAuth, (req, res) => {
  ...
  db.get(verifySql, [booking_id], (err, booking) => {
      if (err) {
          console.error('❌ DB Error:', err.message);
          return res.status(500).json({ error: err.message });
      }
...
app.get('/api/booking/:bookingId', (req, res) => {
  const sql = `SELECT seat_number, student_name, status FROM seat_bookings WHERE trip_id = ?`;
  db.all(sql, [req.params.tripId], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    const names = {};
    rows.forEach(r => { names[r.seat_number] = r.student_name; });
    res.json(names);
  });
});
